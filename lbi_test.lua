...
function execute_bytecode(BCode, Env)
...
function luaX:next(ls)
	...
	if ls.lookahead.token ~= "TK_EOS" then  -- is there a look-ahead token?
		...
		--print("luaX:next is setting token")
		ls.t.token = self:llex(ls, ls.t)  -- read next token
		--print("luaX:next set token")
	end
end
...
function luaX:lookahead(ls)
	--print("lookahead is setting token...")
	-- lua_assert(ls.lookahead.token == "TK_EOS")
	ls.lookahead.token = self:llex(ls, ls.lookahead)
	--print("lookahead set token")
end
...
function luaX:llex(ls, Token)
	--print("luaX:llex, Token: "..tostring(Token))
	ls.buff = ""
	while true do
		local c = ls.current
		----------------------------------------------------------------
...
-- AFTER USING luaK, luaY will get set to this??
...
function newcclosure(f) return coroutine.wrap(f) end --[[ xD!!!!! UNC support 3% ]]
function printbanana() print'banana' end
function isluau() return true end
function getrenv() return renv end
function getgenv() return getfenv() end
...
rawset(script_env, "script", script)
rawset(script_env, "loadstring", load_string)
rawset(script_env, "httpget", httpget)
rawset(script_env, "newcclosure", newcclosure)
rawset(script_env, "printbanana", printbanana)
rawset(script_env, "isluau", isluau)
...
function load_string(str,env)
	...
	local env = env or script_env or getfenv(2)
	local name = "Script0";
	local ran,error = pcall(function()
		--print("[load_string]: Creating string for luaZ buffer...");
		local zio = luaZ:init(luaZ:make_getS(str), nil)
		if not zio then return error() end
		--print("[load_string]: Parsing data into closure...");
		local func = luaY:parser(LuaState, zio, nil, name)
		--print("[load_string]: Parsed");
		writer, buff = luaU:make_setS()
		--print("[load_string]: Receiving bytecode output...");
		luaU:dump(LuaState, func, writer, buff)
		--warn("Running script `"..name.."`");
		f = execute_bytecode(buff.data, env)
	end)
...
while task.wait(.1) do
    if celeryexec.Value ~= celerytoken then
		task.spawn(function()
			local func = load_string(celeryexec.Value, script_env);
